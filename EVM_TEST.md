...
| **R2** | **`PWR_EN`(PA0) = LTC3895 12V 컨버터의 `RUN` 핀인데, 펌웨어가 한 번도 High로 올리지 않는다.** 아래 상세 | **회로 확인 필요 — 우선순위 높음** |
...
- [ ] 부팅 후 `BMS_RTC_ClockSource()` == `BMS_RTC_CLK_LSE`(1) 인지
      — `BMS_RTC_IsReady()` 는 **LSI 폴백일 때도 1** 이라 LSE 성공 판정에 쓸 수 없다.
      2(=LSI)면 ±5% 오차 = 3일 임계가 ±3.6시간, 1시간 임계가 ±3분 틀어진다
- [ ] `.ioc` 의 `HSE_VALUE` 를 16 MHz로 정정 (`STM32CUBEMX_TODO.md` §3-2)

### 🌡 TS1 서미스터 부착 위치 확인 (온도 임계의 전제)

`B_BMS_init.c` 의 OTC/OTD 임계는 **TS1이 팩에서 가장 뜨거운 셀의 표면에 붙어 있다**는 전제로
INR21700-50S 데이터시트 **3.15(표면)** 행을 쓰고 있다. 배치가 바뀌면 허용 범위가 바뀐다:

| TS1 위치 | 방전 허용 | OTD 65도 |
|---|---|---|
| 셀 표면 (Note *2 준수) | -20~80도 | ✅ 유효 |
| 팩 내부 공기 | -20~**60**도 | 🔴 초과 — 60도 이하로 내리거나 ΔT 실측 후 보정 |

- [ ] TS1이 **셀 표면에 접촉**해 있는지 (공기 중에 떠 있지 않은지) 육안 확인
- [ ] 여러 셀 중 **온도 상승이 가장 큰 위치**에 붙었는지 (Note *2 요구사항)
- [ ] 공기를 재는 배치라면 최대 전류 방전 중 **공기 ↔ 셀 표면 ΔT 실측** 후 임계 재산정
      (계산으로는 정할 수 없음)
- [ ] TS3(FET 온도)가 TOP 보드의 FET 근처에 있는지
...
```
U11 STM32U545CEU6Q  PA0 (PWR_EN)
      │   (직결 — 중간에 직렬 저항/분압 없음)
      ▼
U9 LTC3895EFE-PBF  pin 34 = RUN
```

LTC3895의 `RUN`은 컨버터 인에이블이다 (보고서 "12V 컨버터" 절: *"RUN 1.2V 이상 시 스위칭 루프 동작"*).
그런데 `gpio.c`는 초기화에서 `PWR_EN_Pin`을 **`GPIO_PIN_RESET`(Low)** 으로 쓰고,
그 뒤 코드 어디에서도 High로 올리지 않는다.

**즉 펌웨어가 배터리 유래 12V 컨버터를 계속 꺼둔 상태로 동작한다.**

보고서 "1. 전력흐름"이 *"외부 12V 컨버터에서 전원을 받으면 Relay를 통해서 배터리와 외부 라인 연결"*
이라고 되어 있으므로, 의도는 아마 이것으로 보인다 (**추정 — 확인 필요**):

1. 외부 12 V로 보드가 먼저 살아난다
2. MCU가 부팅해서 BQ 초기화·안전 확인을 끝낸다
3. MCU가 `PWR_EN`을 High로 올려 배터리 유래 12 V 컨버터를 켠다
4. 이제 외부 전원 없이 자립 동작한다

이게 맞다면 **3번이 구현되어 있지 않다.** 외부 12 V를 떼는 순간 보드가 죽는다.

- [ ] `PWR_EN` = LTC3895 RUN 이 맞는지 회로도에서 직접 확인
- [ ] RUN 에 BAT_IN 쪽 풀업/분압이 따로 있는지 (KiCad상으로는 안 보였다 — 네트가 MCU와 RUN 두 점뿐)
- [ ] 위 1~4 시퀀스가 의도한 설계가 맞는지
- [ ] 맞다면 **어느 시점에** High로 올려야 하는지 결정
      (BQ 초기화 성공 확인 후? BOTHOFF 해제와 같은 시점? PF 발생 시 다시 Low로 내려야 하나?)

> ⚠️ 이건 **전원 시퀀싱**이라 잘못 넣으면 보드가 자기 전원을 끊는다.
> 회로 확인 전에는 코드를 고치지 않았다.
